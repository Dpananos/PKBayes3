functions{
  vector concentration(vector t, vector D, vector F, vector V, vector ka, vector ke){
    ...
    vector concentration2(vector t, real D, real F, real V, real ka, real ke){
    return D * F * ka / (V * (ke - ka)) * (exp(-ka * t) - exp(-ke * t));
  }
}
data{
...
transformed parameters{
  vector<lower=0>[n] V = exp(mu_V + b_sex_V*sex + b_weight_V*scaled_weight);
  ...
  vector<lower=0, upper=1>[n] a = inv_logit(mu_a + b_sex_a*sex + b_weight_a*scaled_weight);
...
model{
  mu_V ~ cauchy(log(21), 1);
  b_sex_V ~ cauchy(0, 0.25);
  b_weight_V ~ cauchy(0, 1);
  ...
  b_weight_a ~ normal(0, 0.25);
  
  mu_F ~ normal(0, 0.125);
  b_amiodarone_F ~ cauchy(0, tau_F);
  b_diltiazem_F ~ cauchy(0, tau_F);
  tau_F ~ normal(0,1);
