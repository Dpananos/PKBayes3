...
transformed parameters{
  ...
  vector<lower=0>[r_n_subjectids] r_ke = exp(mu_ke + z_ke*s_ke + b_ke_age*r_scaled_age + b_ke_weight*r_scaled_weight + b_ke_is_male*r_is_male + b_ke_creatinine*r_scaled_creatinine );
  vector<lower=0, upper=1>[r_n_subjectids] r_alpha = inv_logit(mu_alpha + z_alpha*s_alpha);
  vector<lower=0>[r_n_subjectids] r_ka = r_ke ./ r_alpha;
...
model{
  ...
  b_ke_age ~ normal(0, 0.25);
  b_ke_weight ~ normal(0, 0.25);
...
generated quantities{
  vector[r_n_subjectids] r_tmax = log(r_ka ./r_ke) ./ (r_ka-r_ke);
  vector[u_n] u_tmax = log(u_ka ./ u_ke) ./ (u_ka-u_ke);
