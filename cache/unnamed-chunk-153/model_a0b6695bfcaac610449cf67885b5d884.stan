...
transformed parameters{
 ...
 vector<lower=0>[n] ke = exp(mu_ke + b_creatinine_ke*scaled_creatinine + b_weight_ke*scaled_weight);
 vector<lower=0>[n] ka= ke/a;
 vector[n] C0 = rep_vector(0.0, n);
 vector[n] C = rep_vector(0.0, n);
 
 for(i in 0:10){
   C0 = C0 + concentration(rep_vector(12.0*i, n), dose_mg_twice_daily, F, rep_vector(V, n), ka, ke);
 }
 C = C0 + concentration(hrs_post_dose, dose_mg_twice_daily, F, rep_vector(V, n), ka, ke);
}
model{
  V ~ normal(21.0, 1.0);
