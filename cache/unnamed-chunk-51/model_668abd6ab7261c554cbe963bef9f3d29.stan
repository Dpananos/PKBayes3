...
transformed parameters{
 ...
 real<lower=0> ka= -log(a)/(tmax*(1-a));
 vector<lower=0>[n] C0 = rep_vector(0.0, n);
 vector[n] C;
 
 for(i in 1:5){
   C0 = C0 + concentration(rep_vector(12*i, n), dose_mg_twice_daily, F, V, rep_vector(ka, n), ke);
 }
 
  C = C0 + concentration(hrs_post_dose, dose_mg_twice_daily, F, V, rep_vector(ka, n), ke);
}
model{
  mu_V ~ normal(log(5), .5);
