...
vector system(vector y, vector theta, real[] x_r, int[] x_i ){
  ...
  z[1] = theta[1] - sqrt(y[3]/y[2]);
  z[2] = theta[2] - D*exp(y[1] - 2*sqrt(y[2]*y[3]));
  z[3] = theta[3] - sqrt(y[2]/y[3]) / (2*exp(y[1]) * bessel_second_kind(1, 2*sqrt(y[2]*y[3])));
...
vector[n] history_heart_failure;
...
transformed data{
  vector[3] b_guess = to_vector({1.0, 1.0, 1.0});
...
transformed parameters{
  ...
  for(i in 1:n){
    ...
    b = algebra_solver(system, b_guess, theta, {dose_mg_twice_daily[i]}, x_i); 
    E_y[i] = dose_mg_twice_daily[i] * exp(b[1] + b[2]*hrs_post_dose[i] + b[3]/hrs_post_dose[i]);
  }
}
model{
