...
transformed parameters{
 vector<lower=0>[n] ke = mu_ke*exp(b_creatinine_ke*creatinine_micromol_l);
 vector<lower=0>[n] ka= ke/a;
 vector[n] C = concentration(hrs_post_dose, dose_mg_twice_daily, rep_vector(F, n), rep_vector(V, n), rep_vector(ka, n), ke);
}
model{
  V ~ normal(21.0, 1.0);
